...
})({"mcLg6":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "4a236f9275d0a351";
module.bundle.HMR_BUNDLE_ID = "cf9f3ad89c26cd0e";
"use strict";
...
},{}],"dRvE2":[function(require,module,exports) {
(function(global, factory) {
    ...
    ***************************************************************************** */ /* global Reflect, Promise */ var extendStatics = function(d1, b1) {
    ...
    var BACKGROUND_CLIP1;
    (function(BACKGROUND_CLIP) {
        BACKGROUND_CLIP[BACKGROUND_CLIP["BORDER_BOX"] = 0] = "BORDER_BOX";
        BACKGROUND_CLIP[BACKGROUND_CLIP["PADDING_BOX"] = 1] = "PADDING_BOX";
        BACKGROUND_CLIP[BACKGROUND_CLIP["CONTENT_BOX"] = 2] = "CONTENT_BOX";
    })(BACKGROUND_CLIP1 || (BACKGROUND_CLIP1 = {
    }));
    var backgroundClip1 = {
        ...
        parse: function(_context, tokens) {
            return tokens.map(function(token) {
                if (isIdentToken(token)) switch(token.value){
                    case 'padding-box':
                        return BACKGROUND_CLIP1.PADDING_BOX;
                    case 'content-box':
                        return BACKGROUND_CLIP1.CONTENT_BOX;
                }
                return BACKGROUND_CLIP1.BORDER_BOX;
            });
        }
    };
    ...
    var calculateRadius = function(gradient, x, y, width, height) {
        ...
        switch(gradient.size){
            case CSSRadialExtent1.CLOSEST_SIDE:
                // The ending shape is sized so that that it exactly meets the side of the gradient box closest to the gradient’s center.
                // If the shape is an ellipse, it exactly meets the closest side in each dimension.
                if (gradient.shape === CSSRadialShape1.CIRCLE) rx = ry = Math.min(Math.abs(x), Math.abs(x - width), Math.abs(y), Math.abs(y - height));
                else if (gradient.shape === CSSRadialShape1.ELLIPSE) {
                    rx = Math.min(Math.abs(x), Math.abs(x - width));
                    ry = Math.min(Math.abs(y), Math.abs(y - height));
                }
                break;
            case CSSRadialExtent1.CLOSEST_CORNER:
                // The ending shape is sized so that that it passes through the corner of the gradient box closest to the gradient’s center.
                // If the shape is an ellipse, the ending shape is given the same aspect-ratio it would have if closest-side were specified.
                if (gradient.shape === CSSRadialShape1.CIRCLE) rx = ry = Math.min(distance(x, y), distance(x, y - height), distance(x - width, y), distance(x - width, y - height));
                else if (gradient.shape === CSSRadialShape1.ELLIPSE) {
                ...
                break;
            case CSSRadialExtent1.FARTHEST_SIDE:
                // Same as closest-side, except the ending shape is sized based on the farthest side(s)
                if (gradient.shape === CSSRadialShape1.CIRCLE) rx = ry = Math.max(Math.abs(x), Math.abs(x - width), Math.abs(y), Math.abs(y - height));
                else if (gradient.shape === CSSRadialShape1.ELLIPSE) {
                    rx = Math.max(Math.abs(x), Math.abs(x - width));
                    ry = Math.max(Math.abs(y), Math.abs(y - height));
                }
                break;
            case CSSRadialExtent1.FARTHEST_CORNER:
                // Same as closest-corner, except the ending shape is sized based on the farthest corner.
                // If the shape is an ellipse, the ending shape is given the same aspect ratio it would have if farthest-side were specified.
                if (gradient.shape === CSSRadialShape1.CIRCLE) rx = ry = Math.max(distance(x, y), distance(x, y - height), distance(x - width, y), distance(x - width, y - height));
                else if (gradient.shape === CSSRadialShape1.ELLIPSE) {
    ...
    var linearGradient = function(context, tokens) {
        ...
        return {
            angle: angle$1,
            stops: stops,
            type: CSSImageType1.LINEAR_GRADIENT
        };
    };
    ...
    var webkitGradient = function(context, tokens) {
        var angle = deg1(180);
        var stops = [];
        var type = CSSImageType1.LINEAR_GRADIENT;
        var shape = CSSRadialShape1.CIRCLE;
        var size = CSSRadialExtent1.FARTHEST_CORNER;
        var position = [];
        parseFunctionArgs(tokens).forEach(function(arg, i) {
            var firstToken = arg[0];
            if (i === 0) {
                if (isIdentToken(firstToken) && firstToken.value === 'linear') {
                    ...
                    type = CSSImageType1.RADIAL_GRADIENT;
                    return;
                }
            }
        ...
        return type === CSSImageType1.LINEAR_GRADIENT ? {
            ...
            if (i === 0) {
                ...
                isColorStop = arg.reduce(function(acc, token) {
                    if (isAtPosition_1) {
                        ...
                        case CIRCLE:
                            shape = CSSRadialShape1.CIRCLE;
                            return false;
                        case ELLIPSE:
                            shape = CSSRadialShape1.ELLIPSE;
                            return false;
                        case 'at':
                            isAtPosition_1 = true;
                            return false;
                        case CLOSEST_SIDE:
                            size = CSSRadialExtent1.CLOSEST_SIDE;
                            return false;
                        case COVER:
                        case FARTHEST_SIDE:
                            size = CSSRadialExtent1.FARTHEST_SIDE;
                            return false;
                        case CONTAIN:
                        case CLOSEST_CORNER:
                            size = CSSRadialExtent1.CLOSEST_CORNER;
                            return false;
                        case FARTHEST_CORNER:
                            size = CSSRadialExtent1.FARTHEST_CORNER;
                            return false;
                    }
    ...
    })(CSSImageType1 || (CSSImageType1 = {
    }));
    var isLinearGradient = function(background) {
        return background.type === CSSImageType1.LINEAR_GRADIENT;
    };
    var isRadialGradient = function(background) {
        return background.type === CSSImageType1.RADIAL_GRADIENT;
    };
    var CSSRadialShape1;
    (function(CSSRadialShape) {
        CSSRadialShape[CSSRadialShape["CIRCLE"] = 0] = "CIRCLE";
        CSSRadialShape[CSSRadialShape["ELLIPSE"] = 1] = "ELLIPSE";
    })(CSSRadialShape1 || (CSSRadialShape1 = {
    }));
    var CSSRadialExtent1;
    (function(CSSRadialExtent) {
        CSSRadialExtent[CSSRadialExtent["CLOSEST_SIDE"] = 0] = "CLOSEST_SIDE";
        CSSRadialExtent[CSSRadialExtent["FARTHEST_SIDE"] = 1] = "FARTHEST_SIDE";
        CSSRadialExtent[CSSRadialExtent["CLOSEST_CORNER"] = 2] = "CLOSEST_CORNER";
        CSSRadialExtent[CSSRadialExtent["FARTHEST_CORNER"] = 3] = "FARTHEST_CORNER";
    })(CSSRadialExtent1 || (CSSRadialExtent1 = {
    }));
    var image1 = {
        name: 'image',
        parse: function(context, value) {
    ...
    var BACKGROUND_REPEAT1;
    (function(BACKGROUND_REPEAT) {
        BACKGROUND_REPEAT[BACKGROUND_REPEAT["REPEAT"] = 0] = "REPEAT";
        BACKGROUND_REPEAT[BACKGROUND_REPEAT["NO_REPEAT"] = 1] = "NO_REPEAT";
        BACKGROUND_REPEAT[BACKGROUND_REPEAT["REPEAT_X"] = 2] = "REPEAT_X";
        BACKGROUND_REPEAT[BACKGROUND_REPEAT["REPEAT_Y"] = 3] = "REPEAT_Y";
    })(BACKGROUND_REPEAT1 || (BACKGROUND_REPEAT1 = {
    }));
    ...
    var parseBackgroundRepeat = function(value) {
        switch(value){
            case 'no-repeat':
                return BACKGROUND_REPEAT1.NO_REPEAT;
            case 'repeat-x':
            case 'repeat no-repeat':
                return BACKGROUND_REPEAT1.REPEAT_X;
            case 'repeat-y':
            case 'no-repeat repeat':
                return BACKGROUND_REPEAT1.REPEAT_Y;
            case 'repeat':
            default:
                return BACKGROUND_REPEAT1.REPEAT;
        }
    };
    ...
    var borderStyleForSide = function(side) {
        return {
            ...
            parse: function(_context, style) {
                switch(style){
                    case 'none':
                        return BORDER_STYLE1.NONE;
                    case 'dashed':
                        return BORDER_STYLE1.DASHED;
                    case 'dotted':
                        return BORDER_STYLE1.DOTTED;
                    case 'double':
                        return BORDER_STYLE1.DOUBLE;
                }
                return BORDER_STYLE1.SOLID;
            }
        };
    };
        ...
        parse: function(_context, float) {
            switch(float){
                ...
                case 'right':
                    return FLOAT1.RIGHT;
                case 'inline-start':
                    return FLOAT1.INLINE_START;
                case 'inline-end':
                    return FLOAT1.INLINE_END;
            }
            return FLOAT1.NONE;
        }
    };
    ...
    var LIST_STYLE_POSITION1;
    (function(LIST_STYLE_POSITION) {
        LIST_STYLE_POSITION[LIST_STYLE_POSITION["INSIDE"] = 0] = "INSIDE";
        LIST_STYLE_POSITION[LIST_STYLE_POSITION["OUTSIDE"] = 1] = "OUTSIDE";
    })(LIST_STYLE_POSITION1 || (LIST_STYLE_POSITION1 = {
    }));
    var listStylePosition = {
        ...
        parse: function(_context, position) {
            switch(position){
                case 'inside':
                    return LIST_STYLE_POSITION1.INSIDE;
                case 'outside':
                default:
                    return LIST_STYLE_POSITION1.OUTSIDE;
            }
        }
    };
    var LIST_STYLE_TYPE1;
    ...
    })(LIST_STYLE_TYPE1 || (LIST_STYLE_TYPE1 = {
    }));
    var listStyleType = {
        ...
        parse: function(_context, type) {
            switch(type){
                case 'disc':
                    return LIST_STYLE_TYPE1.DISC;
                case 'circle':
                    return LIST_STYLE_TYPE1.CIRCLE;
                case 'square':
                    return LIST_STYLE_TYPE1.SQUARE;
                case 'decimal':
                    return LIST_STYLE_TYPE1.DECIMAL;
                case 'cjk-decimal':
                    return LIST_STYLE_TYPE1.CJK_DECIMAL;
                case 'decimal-leading-zero':
                    return LIST_STYLE_TYPE1.DECIMAL_LEADING_ZERO;
                case 'lower-roman':
                    return LIST_STYLE_TYPE1.LOWER_ROMAN;
                case 'upper-roman':
                    return LIST_STYLE_TYPE1.UPPER_ROMAN;
                case 'lower-greek':
                    return LIST_STYLE_TYPE1.LOWER_GREEK;
                case 'lower-alpha':
                    return LIST_STYLE_TYPE1.LOWER_ALPHA;
                case 'upper-alpha':
                    return LIST_STYLE_TYPE1.UPPER_ALPHA;
                case 'arabic-indic':
                    return LIST_STYLE_TYPE1.ARABIC_INDIC;
                case 'armenian':
                    return LIST_STYLE_TYPE1.ARMENIAN;
                case 'bengali':
                    return LIST_STYLE_TYPE1.BENGALI;
                case 'cambodian':
                    return LIST_STYLE_TYPE1.CAMBODIAN;
                case 'cjk-earthly-branch':
                    return LIST_STYLE_TYPE1.CJK_EARTHLY_BRANCH;
                case 'cjk-heavenly-stem':
                    return LIST_STYLE_TYPE1.CJK_HEAVENLY_STEM;
                case 'cjk-ideographic':
                    return LIST_STYLE_TYPE1.CJK_IDEOGRAPHIC;
                case 'devanagari':
                    return LIST_STYLE_TYPE1.DEVANAGARI;
                case 'ethiopic-numeric':
                    return LIST_STYLE_TYPE1.ETHIOPIC_NUMERIC;
                case 'georgian':
                    return LIST_STYLE_TYPE1.GEORGIAN;
                case 'gujarati':
                    return LIST_STYLE_TYPE1.GUJARATI;
                case 'gurmukhi':
                    return LIST_STYLE_TYPE1.GURMUKHI;
                case 'hebrew':
                    return LIST_STYLE_TYPE1.HEBREW;
                case 'hiragana':
                    return LIST_STYLE_TYPE1.HIRAGANA;
                case 'hiragana-iroha':
                    return LIST_STYLE_TYPE1.HIRAGANA_IROHA;
                case 'japanese-formal':
                    return LIST_STYLE_TYPE1.JAPANESE_FORMAL;
                case 'japanese-informal':
                    return LIST_STYLE_TYPE1.JAPANESE_INFORMAL;
                case 'kannada':
                    return LIST_STYLE_TYPE1.KANNADA;
                case 'katakana':
                    return LIST_STYLE_TYPE1.KATAKANA;
                case 'katakana-iroha':
                    return LIST_STYLE_TYPE1.KATAKANA_IROHA;
                case 'khmer':
                    return LIST_STYLE_TYPE1.KHMER;
                case 'korean-hangul-formal':
                    return LIST_STYLE_TYPE1.KOREAN_HANGUL_FORMAL;
                case 'korean-hanja-formal':
                    return LIST_STYLE_TYPE1.KOREAN_HANJA_FORMAL;
                case 'korean-hanja-informal':
                    return LIST_STYLE_TYPE1.KOREAN_HANJA_INFORMAL;
                case 'lao':
                    return LIST_STYLE_TYPE1.LAO;
                case 'lower-armenian':
                    return LIST_STYLE_TYPE1.LOWER_ARMENIAN;
                case 'malayalam':
                    return LIST_STYLE_TYPE1.MALAYALAM;
                case 'mongolian':
                    return LIST_STYLE_TYPE1.MONGOLIAN;
                case 'myanmar':
                    return LIST_STYLE_TYPE1.MYANMAR;
                case 'oriya':
                    return LIST_STYLE_TYPE1.ORIYA;
                case 'persian':
                    return LIST_STYLE_TYPE1.PERSIAN;
                case 'simp-chinese-formal':
                    return LIST_STYLE_TYPE1.SIMP_CHINESE_FORMAL;
                case 'simp-chinese-informal':
                    return LIST_STYLE_TYPE1.SIMP_CHINESE_INFORMAL;
                case 'tamil':
                    return LIST_STYLE_TYPE1.TAMIL;
                case 'telugu':
                    return LIST_STYLE_TYPE1.TELUGU;
                case 'thai':
                    return LIST_STYLE_TYPE1.THAI;
                case 'tibetan':
                    return LIST_STYLE_TYPE1.TIBETAN;
                case 'trad-chinese-formal':
                    return LIST_STYLE_TYPE1.TRAD_CHINESE_FORMAL;
                case 'trad-chinese-informal':
                    return LIST_STYLE_TYPE1.TRAD_CHINESE_INFORMAL;
                case 'upper-armenian':
                    return LIST_STYLE_TYPE1.UPPER_ARMENIAN;
                case 'disclosure-open':
                    return LIST_STYLE_TYPE1.DISCLOSURE_OPEN;
                case 'disclosure-closed':
                    return LIST_STYLE_TYPE1.DISCLOSURE_CLOSED;
                case 'none':
                default:
                    return LIST_STYLE_TYPE1.NONE;
            }
        }
    };
        ...
        parse: function(_context, tokens) {
            return tokens.filter(isIdentToken).map(function(overflow) {
                switch(overflow.value){
                    case 'hidden':
                        return OVERFLOW1.HIDDEN;
                    case 'scroll':
                        return OVERFLOW1.SCROLL;
                    case 'clip':
                        return OVERFLOW1.CLIP;
                    case 'auto':
                        return OVERFLOW1.AUTO;
                    case 'visible':
                    default:
                        return OVERFLOW1.VISIBLE;
                }
            });
        }
    };
    ...
    var textAlign1 = {
        ...
        parse: function(_context, textAlign) {
            switch(textAlign){
                case 'right':
                    return TEXT_ALIGN1.RIGHT;
                case 'center':
                case 'justify':
                    return TEXT_ALIGN1.CENTER;
                case 'left':
                default:
                    return TEXT_ALIGN1.LEFT;
            }
        }
    };
    var POSITION1;
    ...
    var position1 = {
        ...
        parse: function(_context, position) {
            switch(position){
                case 'relative':
                    return POSITION1.RELATIVE;
                case 'absolute':
                    return POSITION1.ABSOLUTE;
                case 'fixed':
                    return POSITION1.FIXED;
                case 'sticky':
                    return POSITION1.STICKY;
            }
            return POSITION1.STATIC;
        }
    };
    var textShadow1 = {
        name: 'text-shadow',
    ...
    })(TEXT_TRANSFORM1 || (TEXT_TRANSFORM1 = {
    }));
    var textTransform1 = {
        name: 'text-transform',
        initialValue: 'none',
        ...
        parse: function(_context, textTransform) {
            switch(textTransform){
                case 'uppercase':
                    return TEXT_TRANSFORM1.UPPERCASE;
                case 'lowercase':
                    return TEXT_TRANSFORM1.LOWERCASE;
                case 'capitalize':
                    return TEXT_TRANSFORM1.CAPITALIZE;
            }
            return TEXT_TRANSFORM1.NONE;
        }
    };
    var transform$1 = {
        name: 'transform',
    ...
    var visibility1 = {
        ...
        parse: function(_context, visibility) {
            switch(visibility){
                case 'hidden':
                    return VISIBILITY1.HIDDEN;
                case 'collapse':
                    return VISIBILITY1.COLLAPSE;
                case 'visible':
                default:
                    return VISIBILITY1.VISIBLE;
            }
        }
    };
    var WORD_BREAK1;
    ...
    var fontStyle = {
        ...
        parse: function(_context, overflow) {
            switch(overflow){
                case 'oblique':
                    return FONT_STYLE1.OBLIQUE;
                case 'italic':
                    return FONT_STYLE1.ITALIC;
                case 'normal':
                default:
                    return FONT_STYLE1.NORMAL;
            }
        }
    };
    ...
    })(PAINT_ORDER_LAYER1 || (PAINT_ORDER_LAYER1 = {
    }));
    var paintOrder1 = {
        name: 'paint-order',
        initialValue: 'normal',
        ...
        parse: function(_context, tokens) {
            var DEFAULT_VALUE = [
                PAINT_ORDER_LAYER1.FILL,
                PAINT_ORDER_LAYER1.STROKE,
                PAINT_ORDER_LAYER1.MARKERS
            ];
            var layers = [];
            tokens.filter(isIdentToken).forEach(function(token) {
                switch(token.value){
                    case 'stroke':
                        layers.push(PAINT_ORDER_LAYER1.STROKE);
                        break;
                    case 'fill':
                        layers.push(PAINT_ORDER_LAYER1.FILL);
                        break;
                    case 'markers':
                        layers.push(PAINT_ORDER_LAYER1.MARKERS);
                        break;
                }
            });
    ...
    var CSSParsedDeclaration1 = function() {
        ...
        CSSParsedDeclaration.prototype.isVisible = function() {
            return this.display > 0 && this.opacity > 0 && this.visibility === VISIBILITY1.VISIBLE;
        };
        ...
        CSSParsedDeclaration.prototype.isPositioned = function() {
            return this.position !== POSITION1.STATIC;
        };
        CSSParsedDeclaration.prototype.isPositionedWithZIndex = function() {
            return this.isPositioned() && !this.zIndex.auto;
        };
        CSSParsedDeclaration.prototype.isFloating = function() {
            return this.float !== FLOAT1.NONE;
        };
    ...
    var CSSParsedPseudoDeclaration1 = function() {
    ...
    var CSSParsedCounterDeclaration1 = function() {
    ...
    var ElementContainer1 = function() {
    ...
    var FEATURES = {
        get SUPPORT_RANGE_BOUNDS () {
            var value7 = testRangeBounds(document);
        ...
        textList.forEach(function(text) {
            if (styles.textDecorationLine.length || text.trim().length > 0) {
                if (FEATURES.SUPPORT_RANGE_BOUNDS) {
                    if (!FEATURES.SUPPORT_WORD_BREAKING) textBounds.push(new TextBounds1(text, Bounds1.fromDOMRectList(context, createRange(node, offset, text.length).getClientRects())));
                    else textBounds.push(new TextBounds1(text, getRangeBounds(context, node, offset, text.length)));
                } else {
    ...
    var getRangeBounds = function(context, node, offset, length) {
        return Bounds1.fromClientRect(context, createRange(node, offset, length).getBoundingClientRect());
    };
    ...
    var transform1 = function(text, transform) {
        switch(transform){
            case TEXT_TRANSFORM1.LOWERCASE:
                return text.toLowerCase();
            case TEXT_TRANSFORM1.CAPITALIZE:
                return text.replace(CAPITALIZE, capitalize);
            case TEXT_TRANSFORM1.UPPERCASE:
    ...
    var InputElementContainer1 = function(_super) {
        ...
        function InputElementContainer(context, input) {
            ...
            if (_this.type === CHECKBOX || _this.type === RADIO) {
                ...
                _this.styles.borderTopStyle = _this.styles.borderRightStyle = _this.styles.borderBottomStyle = _this.styles.borderLeftStyle = BORDER_STYLE1.SOLID;
                _this.styles.backgroundClip = [
                    BACKGROUND_CLIP1.BORDER_BOX
                ];
    ...
    var createCJKCounter = function(value, numbers, multipliers, negativeSign, suffix, flags) {
        if (value < -9999 || value > 9999) return createCounterText(value, LIST_STYLE_TYPE1.CJK_DECIMAL, suffix.length > 0);
    ...
    var createCounterText = function(value, type, appendSuffix) {
        ...
        switch(type){
            case LIST_STYLE_TYPE1.DISC:
                return '•' + spaceSuffix;
            case LIST_STYLE_TYPE1.CIRCLE:
                return '◦' + spaceSuffix;
            case LIST_STYLE_TYPE1.SQUARE:
                return '◾' + spaceSuffix;
            case LIST_STYLE_TYPE1.DECIMAL_LEADING_ZERO:
                var string = createCounterStyleFromRange(value, 48, 57, true, defaultSuffix);
                return string.length < 4 ? "0" + string : string;
            case LIST_STYLE_TYPE1.CJK_DECIMAL:
                return createCounterStyleFromSymbols(value, '〇一二三四五六七八九', cjkSuffix);
            case LIST_STYLE_TYPE1.LOWER_ROMAN:
                return createAdditiveCounter(value, 1, 3999, ROMAN_UPPER, LIST_STYLE_TYPE1.DECIMAL, defaultSuffix).toLowerCase();
            case LIST_STYLE_TYPE1.UPPER_ROMAN:
                return createAdditiveCounter(value, 1, 3999, ROMAN_UPPER, LIST_STYLE_TYPE1.DECIMAL, defaultSuffix);
            case LIST_STYLE_TYPE1.LOWER_GREEK:
                return createCounterStyleFromRange(value, 945, 969, false, defaultSuffix);
            case LIST_STYLE_TYPE1.LOWER_ALPHA:
                return createCounterStyleFromRange(value, 97, 122, false, defaultSuffix);
            case LIST_STYLE_TYPE1.UPPER_ALPHA:
                return createCounterStyleFromRange(value, 65, 90, false, defaultSuffix);
            case LIST_STYLE_TYPE1.ARABIC_INDIC:
                return createCounterStyleFromRange(value, 1632, 1641, true, defaultSuffix);
            case LIST_STYLE_TYPE1.ARMENIAN:
            case LIST_STYLE_TYPE1.UPPER_ARMENIAN:
                return createAdditiveCounter(value, 1, 9999, ARMENIAN, LIST_STYLE_TYPE1.DECIMAL, defaultSuffix);
            case LIST_STYLE_TYPE1.LOWER_ARMENIAN:
                return createAdditiveCounter(value, 1, 9999, ARMENIAN, LIST_STYLE_TYPE1.DECIMAL, defaultSuffix).toLowerCase();
            case LIST_STYLE_TYPE1.BENGALI:
                return createCounterStyleFromRange(value, 2534, 2543, true, defaultSuffix);
            case LIST_STYLE_TYPE1.CAMBODIAN:
            case LIST_STYLE_TYPE1.KHMER:
                return createCounterStyleFromRange(value, 6112, 6121, true, defaultSuffix);
            case LIST_STYLE_TYPE1.CJK_EARTHLY_BRANCH:
                return createCounterStyleFromSymbols(value, '子丑寅卯辰巳午未申酉戌亥', cjkSuffix);
            case LIST_STYLE_TYPE1.CJK_HEAVENLY_STEM:
                return createCounterStyleFromSymbols(value, '甲乙丙丁戊己庚辛壬癸', cjkSuffix);
            case LIST_STYLE_TYPE1.CJK_IDEOGRAPHIC:
            case LIST_STYLE_TYPE1.TRAD_CHINESE_INFORMAL:
                return createCJKCounter(value, '零一二三四五六七八九', CHINESE_INFORMAL_MULTIPLIERS, '負', cjkSuffix, CJK_TEN_COEFFICIENTS | CJK_TEN_HIGH_COEFFICIENTS | CJK_HUNDRED_COEFFICIENTS);
            case LIST_STYLE_TYPE1.TRAD_CHINESE_FORMAL:
                return createCJKCounter(value, '零壹貳參肆伍陸柒捌玖', CHINESE_FORMAL_MULTIPLIERS, '負', cjkSuffix, CJK_ZEROS | CJK_TEN_COEFFICIENTS | CJK_TEN_HIGH_COEFFICIENTS | CJK_HUNDRED_COEFFICIENTS);
            case LIST_STYLE_TYPE1.SIMP_CHINESE_INFORMAL:
                return createCJKCounter(value, '零一二三四五六七八九', CHINESE_INFORMAL_MULTIPLIERS, '负', cjkSuffix, CJK_TEN_COEFFICIENTS | CJK_TEN_HIGH_COEFFICIENTS | CJK_HUNDRED_COEFFICIENTS);
            case LIST_STYLE_TYPE1.SIMP_CHINESE_FORMAL:
                return createCJKCounter(value, '零壹贰叁肆伍陆柒捌玖', CHINESE_FORMAL_MULTIPLIERS, '负', cjkSuffix, CJK_ZEROS | CJK_TEN_COEFFICIENTS | CJK_TEN_HIGH_COEFFICIENTS | CJK_HUNDRED_COEFFICIENTS);
            case LIST_STYLE_TYPE1.JAPANESE_INFORMAL:
                return createCJKCounter(value, '〇一二三四五六七八九', '十百千万', JAPANESE_NEGATIVE, cjkSuffix, 0);
            case LIST_STYLE_TYPE1.JAPANESE_FORMAL:
                return createCJKCounter(value, '零壱弐参四伍六七八九', '拾百千万', JAPANESE_NEGATIVE, cjkSuffix, CJK_ZEROS | CJK_TEN_COEFFICIENTS | CJK_TEN_HIGH_COEFFICIENTS);
            case LIST_STYLE_TYPE1.KOREAN_HANGUL_FORMAL:
                return createCJKCounter(value, '영일이삼사오육칠팔구', '십백천만', KOREAN_NEGATIVE, koreanSuffix, CJK_ZEROS | CJK_TEN_COEFFICIENTS | CJK_TEN_HIGH_COEFFICIENTS);
            case LIST_STYLE_TYPE1.KOREAN_HANJA_INFORMAL:
                return createCJKCounter(value, '零一二三四五六七八九', '十百千萬', KOREAN_NEGATIVE, koreanSuffix, 0);
            case LIST_STYLE_TYPE1.KOREAN_HANJA_FORMAL:
                return createCJKCounter(value, '零壹貳參四五六七八九', '拾百千', KOREAN_NEGATIVE, koreanSuffix, CJK_ZEROS | CJK_TEN_COEFFICIENTS | CJK_TEN_HIGH_COEFFICIENTS);
            case LIST_STYLE_TYPE1.DEVANAGARI:
                return createCounterStyleFromRange(value, 2406, 2415, true, defaultSuffix);
            case LIST_STYLE_TYPE1.GEORGIAN:
                return createAdditiveCounter(value, 1, 19999, GEORGIAN, LIST_STYLE_TYPE1.DECIMAL, defaultSuffix);
            case LIST_STYLE_TYPE1.GUJARATI:
                return createCounterStyleFromRange(value, 2790, 2799, true, defaultSuffix);
            case LIST_STYLE_TYPE1.GURMUKHI:
                return createCounterStyleFromRange(value, 2662, 2671, true, defaultSuffix);
            case LIST_STYLE_TYPE1.HEBREW:
                return createAdditiveCounter(value, 1, 10999, HEBREW, LIST_STYLE_TYPE1.DECIMAL, defaultSuffix);
            case LIST_STYLE_TYPE1.HIRAGANA:
                return createCounterStyleFromSymbols(value, 'あいうえおかきくけこさしすせそたちつてとなにぬねのはひふへほまみむめもやゆよらりるれろわゐゑをん');
            case LIST_STYLE_TYPE1.HIRAGANA_IROHA:
                return createCounterStyleFromSymbols(value, 'いろはにほへとちりぬるをわかよたれそつねならむうゐのおくやまけふこえてあさきゆめみしゑひもせす');
            case LIST_STYLE_TYPE1.KANNADA:
                return createCounterStyleFromRange(value, 3302, 3311, true, defaultSuffix);
            case LIST_STYLE_TYPE1.KATAKANA:
                return createCounterStyleFromSymbols(value, 'アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヰヱヲン', cjkSuffix);
            case LIST_STYLE_TYPE1.KATAKANA_IROHA:
                return createCounterStyleFromSymbols(value, 'イロハニホヘトチリヌルヲワカヨタレソツネナラムウヰノオクヤマケフコエテアサキユメミシヱヒモセス', cjkSuffix);
            case LIST_STYLE_TYPE1.LAO:
                return createCounterStyleFromRange(value, 3792, 3801, true, defaultSuffix);
            case LIST_STYLE_TYPE1.MONGOLIAN:
                return createCounterStyleFromRange(value, 6160, 6169, true, defaultSuffix);
            case LIST_STYLE_TYPE1.MYANMAR:
                return createCounterStyleFromRange(value, 4160, 4169, true, defaultSuffix);
            case LIST_STYLE_TYPE1.ORIYA:
                return createCounterStyleFromRange(value, 2918, 2927, true, defaultSuffix);
            case LIST_STYLE_TYPE1.PERSIAN:
                return createCounterStyleFromRange(value, 1776, 1785, true, defaultSuffix);
            case LIST_STYLE_TYPE1.TAMIL:
                return createCounterStyleFromRange(value, 3046, 3055, true, defaultSuffix);
            case LIST_STYLE_TYPE1.TELUGU:
                return createCounterStyleFromRange(value, 3174, 3183, true, defaultSuffix);
            case LIST_STYLE_TYPE1.THAI:
                return createCounterStyleFromRange(value, 3664, 3673, true, defaultSuffix);
            case LIST_STYLE_TYPE1.TIBETAN:
                return createCounterStyleFromRange(value, 3872, 3881, true, defaultSuffix);
            case LIST_STYLE_TYPE1.DECIMAL:
            default:
                return createCounterStyleFromRange(value, 48, 57, true, defaultSuffix);
        }
    };
    var IGNORE_ATTRIBUTE = 'data-html2canvas-ignore';
    var DocumentCloner1 = function() {
        ...
        DocumentCloner.prototype.resolvePseudoContent = function(node, clone, style, pseudoElt) {
            var _this = this;
            if (!style) return;
            var value8 = style.content;
            var document = clone.ownerDocument;
            if (!document || !value8 || value8 === 'none' || value8 === '-moz-alt-content' || style.display === 'none') return;
                ...
                if (token.type === 0 /* STRING_TOKEN */ ) anonymousReplacedElement.appendChild(document.createTextNode(token.value));
                    ...
                    if (token.name === 'attr') {
                        ...
                        if (counter && isIdentToken(counter)) {
                            ...
                            var counterType = counterStyle && isIdentToken(counterStyle) ? listStyleType.parse(_this.context, counterStyle.value) : LIST_STYLE_TYPE1.DECIMAL;
                        ...
                        if (counter && isIdentToken(counter)) {
                            ...
                            var counterType_1 = counterStyle && isIdentToken(counterStyle) ? listStyleType.parse(_this.context, counterStyle.value) : LIST_STYLE_TYPE1.DECIMAL;
    ...
    var BezierCurve1 = function() {
        function BezierCurve(start, startControl, endControl, end) {
            this.type = PathType1.BEZIER_CURVE;
            this.start = start;
    ...
    var isBezierCurve = function(path) {
        return path.type === PathType1.BEZIER_CURVE;
    };
    var BoundCurves1 = function() {
    ...
    var ElementPaint1 = function() {
        function ElementPaint(container, parent) {
            ...
            if (this.container.styles.overflowX !== OVERFLOW1.VISIBLE) {
        ...
        ElementPaint.prototype.getEffects = function(target) {
            ...
            while(parent){
                ...
                if (inFlow || parent.container.styles.position !== POSITION1.STATIC || !parent.parent) {
                    effects.unshift.apply(effects, croplessEffects);
                    inFlow = [
                        POSITION1.ABSOLUTE,
                        POSITION1.FIXED
                    ].indexOf(parent.container.styles.position) === -1;
                    if (parent.container.styles.overflowX !== OVERFLOW1.VISIBLE) {
    ...
    var calculateBackgroundPaintingArea = function(backgroundClip, element) {
        if (backgroundClip === BACKGROUND_CLIP1.BORDER_BOX) return element.bounds;
        if (backgroundClip === BACKGROUND_CLIP1.CONTENT_BOX) return contentBox(element);
        return paddingBox1(element);
    };
    ...
    var calculateBackgroundRepeatPath = function(repeat, _a, _b, backgroundPositioningArea, backgroundPaintingArea) {
        ...
        switch(repeat){
            case BACKGROUND_REPEAT1.REPEAT_X:
                return [
            ...
            case BACKGROUND_REPEAT1.REPEAT_Y:
                return [
            ...
            case BACKGROUND_REPEAT1.NO_REPEAT:
                return [
    ...
    var CanvasRenderer1 = function(_super) {
                        ...
                        paintOrder.forEach(function(paintOrderLayer) {
                            switch(paintOrderLayer){
                                case PAINT_ORDER_LAYER1.FILL:
                                    ...
                                    break;
                                case PAINT_ORDER_LAYER1.STROKE:
            ...
            return __awaiter(this, void 0, void 0, function() {
                ...
                return __generator(this, function(_c) {
                    switch(_c.label){
                        ...
                        case 14:
                            ...
                            if (isTextInputElement(container) && container.value.length) {
                                ...
                                switch(container.styles.textAlign){
                                    case TEXT_ALIGN1.CENTER:
                                        x += bounds.width / 2;
                                        break;
                                    case TEXT_ALIGN1.RIGHT:
                                        x += bounds.width;
                                        break;
                                }
                            ...
                            if (!(img.type === CSSImageType1.URL)) return [
                                3 /*break*/ ,
                                18
                            ];
                            image = void 0;
                        ...
                        case 19:
                            if (paint.listValue && container.styles.listStyleType !== LIST_STYLE_TYPE1.NONE) {
                        ...
                        case 0:
                            ...
                            _loop_1 = function(backgroundImage) {
                                ...
                                return __generator(this, function(_h) {
                                    switch(_h.label){
                                        case 0:
                                            if (!(backgroundImage.type === CSSImageType1.URL)) return [
                                                3 /*break*/ ,
                                                5
                                            ];
                                            image = void 0;
                ...
                return __generator(this, function(_a) {
                    switch(_a.label){
                        ...
                        case 3:
                            ...
                            border = borders_1[_i];
                            if (!(border.style !== BORDER_STYLE1.NONE && !isTransparent(border.color) && border.width > 0)) return [
                                3 /*break*/ ,
                                11
                            ];
                            if (!(border.style === BORDER_STYLE1.DASHED)) return [
                                3 /*break*/ ,
                                5
                            ];
                            return [
                                4 /*yield*/ ,
                                this.renderDashedDottedBorder(border.color, border.width, side, paint.curves, BORDER_STYLE1.DASHED)
                            ];
                        case 4:
                            _a.sent();
                            return [
                        ...
                        case 5:
                            if (!(border.style === BORDER_STYLE1.DOTTED)) return [
                                3 /*break*/ ,
                                7
                            ];
                            return [
                                4 /*yield*/ ,
                                this.renderDashedDottedBorder(border.color, border.width, side, paint.curves, BORDER_STYLE1.DOTTED)
                            ];
                        case 6:
                            _a.sent();
                            return [
                        ...
                        case 7:
                            if (!(border.style === BORDER_STYLE1.DOUBLE)) return [
                                3 /*break*/ ,
                                9
                            ];
                            return [
        ...
        CanvasRenderer.prototype.renderDashedDottedBorder = function(color, width, side, curvePoints, style) {
            return __awaiter(this, void 0, void 0, function() {
                ...
                return __generator(this, function(_a) {
                    ...
                    this.ctx.beginPath();
                    if (style === BORDER_STYLE1.DOTTED) this.formatPath(strokePaths);
                    else this.formatPath(boxPaths.slice(0, 2));
                    dashLength = width < 3 ? width * 3 : width * 2;
                    spaceLength = width < 3 ? width * 2 : width;
                    if (style === BORDER_STYLE1.DOTTED) {
                        dashLength = width;
                        spaceLength = width;
                    }
                    useLineDash = true;
                    ...
                    if (useLineDash) {
                        if (style === BORDER_STYLE1.DOTTED) this.ctx.setLineDash([
                            0,
    ...
    var calculateBackgroundCurvedPaintingArea = function(clip, curves) {
        switch(clip){
            case BACKGROUND_CLIP1.BORDER_BOX:
                return calculateBorderBoxPath(curves);
            case BACKGROUND_CLIP1.CONTENT_BOX:
                return calculateContentBoxPath(curves);
            case BACKGROUND_CLIP1.PADDING_BOX:
            default:
    ...
    var canvasTextAlign = function(textAlign) {
        switch(textAlign){
            ...
            case TEXT_ALIGN1.RIGHT:
                return 'right';
            case TEXT_ALIGN1.LEFT:
            default:
                return 'left';
        }
    };
    var ForeignObjectRenderer1 = function(_super) {
...
},{}]},["mcLg6"], null, "parcelRequirece68")

//# sourceMappingURL=html2canvas.9c26cd0e.js.map
